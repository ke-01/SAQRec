...
class judger(object):
    ...
    def cal_metric(self, preds, labels, metrics):
        ...
        for metric in metrics:
            if metric == "auc":
                ...
                r2=r2_score(np.asarray(labels), np.asarray(preds))
                res["r2"]=round(r2,5)
    ...
    def return_all_metrics(self, preds, labels, metrics):
        '''to conduct t-test, return results without mean pooling'''
        res = dict()
        
        for metric in metrics:
            if metric.startswith("ndcg"):  # format like:  ndcg@2;4;6;8
                ndcg_list = [1, 2]
                ks = metric.split("@")
                if len(ks) > 1:
                    ndcg_list = [int(token) for token in ks[1].split(";")]
